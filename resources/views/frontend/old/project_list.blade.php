                                        ...
                                        <div class="row justify-content-center">
                                            <div class="rld-single-select ml-22">
                                                <select class="select single-select" name="property_for">
                                                    ...
                                                    <option value="sale" {{ request()->get('property_for') == 'sale' ? 'selected' : '' }}>Sale</option>
                                                    <option value="rent" {{ request()->get('property_for') == 'rent' ? 'selected' : '' }}>Rent</option>
                                                    <option value="lease" {{ request()->get('property_for') == 'lease' ? 'selected' : '' }}>Lease</option>
                                                </select>
                                            </div>
                                            <div class="rld-single-select ml-22">
                                                <select class="select single-select" name="property_type">
                                                    ...
                                                    <option value="{{$typeRow->url}}" {{ request()->get('property_type') == $typeRow->url ? 'selected' : '' }}>{{$typeRow->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="rld-single-select mr-lg-3">
                                                <select class="select single-select mr-0" name="location">
                                                    ...
                                                    <option value="{{$cityRow->url}}" {{ request()->get('location') == $cityRow->url ? 'selected' : '' }}>{{$cityRow->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="rld-single-select mr-lg-3">
                                                <select class="select single-select mr-0" name="brand">
                                                    ...
                                                    <option value="{{$brandRow->url}}" {{ request()->get('brand') == $brandRow->url ? 'selected' : '' }}>{{$brandRow->title}}</option>
                                                    @endforeach
                                                </select>
                                            </div>

                                            <div class="col-xl-2 col-lg-2 col-md-4 pl-0">
                                                <a class="btn btn-yellow" href="javascript:;" onclick="$('#seach-form').submit()">Search Now</a>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="landscapes">
                            <div class="project-single">
                                <div class="project-inner project-head">
                                    <div class="homes">
                                        <!-- homes img -->
                                        <a href="{{route('project.details',$dataRow->url)}}" class="homes-img">
                                            <div class="homes-tag button alt featured">{{ucfirst($dataRow->section)}}</div>
                                            @if($dataRow->is_rera_approved == ' 1')
                                            <div class="homes-tag button rera-section">RERA Approved</div>
                                            @endif
                                            <div class="homes-tag button alt sale">{{ucfirst($dataRow->project_for)}}</div>
                                            <img src="{{url('uploads/project/cover/'.$dataRow->cover_image)}}" alt="{{$dataRow->title}}" class="img-responsive">
                                        </a>
                                    </div>
                                    <div class="button-effect">
                                        <a href="{{route('project.details',$dataRow->url)}}" class="btn"><i class="fa fa-link"></i></a>
                                        @if($dataRow->video != NULL)
                                        <a target="_blank" href="{{$dataRow->video}}" class="btn popup-video popup-youtube"><i class="fas fa-video"></i></a>
                                        @endif
                                    </div>
                                </div>
                                <!-- homes content -->
                                <div class="homes-content">
                                    <!-- homes address -->
                                    <h3><a href="{{route('project.details',$dataRow->url)}}">{{$dataRow->title}}</a></h3>
                                    <p class="homes-address m-0">
                                        ...
                                        <span class="brand-name">{{$dataRow->brand->title}}</span>
                                    </p>

                                    <p class="homes-address m-0">
                                        <i class="fa fa-map-marker"></i> &nbsp;<span>{{$dataRow->address}}</span>
                                    </p>

                                    <p class="homes-address mb-3">
                                        <i class="fa fa-building"></i>&nbsp;
                                        <span>{{$dataRow->type->title}}</span>
                                    </p>
                                    <div class="price-properties footer pt-3 pb-0">
                                        <h3 class="title mt-3">
                                            <a href="{{route('project.details',$dataRow->url)}}">₹ {{$dataRow->price}}</a>
                                        </h3>
                                        <div class="compare">
                                            <a href="{{route('project.details',$dataRow->url)}}" title="Compare">
